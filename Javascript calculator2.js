...
const NumberInput = () => {
        ...
        const getpreviousAndNextNumbers = (currentMethod, bodmasNumber) => {
            ...
            if (currentMethodPosition == 1 && inputMethods.length <= 3) {
                ...
                if (currentMethodPosition == 1 && bodmasNumber.length == 3) {
                    ...
                    const previousNumber = bodmasNumber[0];
                    const nextNumber = bodmasNumber[2];
                    const CollectedNumbers = pre: 200,
                }, {
                    item: "Liqour",
                    price: 100,
                    stock: 200,
        ...
        return ( <
                ...
                <
                div id = 'display'
                style = { displayStyle } > { /*<div>{itemsToProcess}</div>*/ } <
                div > { reduceMultipleZerosAndDecimalsToOne(items.split(',')) } < /div> { /*<div>{InpuItemsToProcess}</div>*/ } <
                div > { answer } < /div> <
                /div>

                <
                div style = { inputMethodStyle } > {
                    methods.map((method) => ( < button id = { method.id }
                            onClick = {
                                () => getCurrentMethod(method.method) } > { method.method } < /button>))
                        } <
                        button id = 'equals'
                        onClick = {
                            () => equals() } >= < /button> <
                        ...
                        div style = { inputNumberStyle } > {
                            numbers.map((number) => ( <
                                    button id = { number.id }
                                    onClick = {
                                        () => getCurrentNumber(number.digit) } > { number.digit } < /button>))
                                }
